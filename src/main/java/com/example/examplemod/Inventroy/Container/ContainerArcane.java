...
public class ContainerArcane extends ContainerMain {
    ...
    public ContainerArcane(InventoryPlayer playerInventory, TileArcane te) {
        ...
        addSlotToContainer(new Slotwand(te,0,20,60));
        addSlotToContainer(new SlotResult(te,1,40,90));
        for(int i = 0; i < 3; ++i) {
            for(int j = 0; j < 3; ++j) {
                addSlotToContainer(new SlotMatrix(te, j + i * 3 + 2, j * 18, i * 18));
            }
        }
        for(int i = 0; i < 2; ++i) {
            for(int j = 0; j < 9; ++j) {
                addSlotToContainer(new Slot(te, j + i * 9 + 11, j * 18,+ i * 18));
            }
        }
        this.te = te;
